...
export default function MeshText({
    ...
    const forceRef = useRef<number>(force / 10)
    ...
    useEffect(() => {
        ...
        const rebuildTex = async () => {
            ...
            // Read actual computed styles from the wrapper so we match
            // whatever CSS font-size / font-family is set on the parent.
            const cs = window.getComputedStyle(wrapper)
            const realSize = parseFloat(cs.fontSize) * dpr
            const realFamily = cs.fontFamily
            const realWeight = cs.fontWeight
            const realStyle = cs.fontStyle
            const realSpacing = cs.letterSpacing

            try {
                if (typeof document !== "undefined") {
                    const fontStr = `${realStyle} ${realWeight} ${realSize}px ${realFamily}`
            ...
            if ((ctx as any).letterSpacing !== undefined) (ctx as any).letterSpacing = realSpacing
            ctx.font = `${realStyle} ${realWeight} ${realSize}px ${realFamily}`
        ...
        const tick = () => {
            ...
            for (let i = 0; i < vertCount; i++) {
                ...
                const dx = disp[i2]; const dy = disp[i2 + 1]
                const cx = cursor.x - (px + dx); const cy = cursor.y - (py + dy)
                const cd = Math.hypot(cx, cy)
                const proximity = Math.max(0, 1 / (1 + cd / 0.05) - 0.1)
                let vx = vel[i2]; let vy = vel[i2 + 1]
                ...
                let ndx = dx + vx * DT; let ndy = dy + vy * DT
            ...
            if (cols.length === 1) { cA = cols[0]; cB = cols[0] }
                ...
                cA = cols[i]; cB = cols[(i + 1) % cols.length]
            }
    ...
    return (
        <div ref={wrapperRef} style={{ position: "absolute", inset: 0, overflow: "hidden", userSelect: "none" }}>
